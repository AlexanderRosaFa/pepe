...
// ── Background themes ─────────────────────────────────────────
const THEMES = [
  { id:"chalk", name:"Tiza",    bg:"#F7F4EE", emoji:"⚪" },
  { id:"mint",  name:"Menta",   bg:"#E6F5EE", emoji:"🟢" },
  { id:"sky",   name:"Cielo",   bg:"#E8F1FB", emoji:"🔵" },
  { id:"peach", name:"Durazno", bg:"#FCEEE3", emoji:"🟠" },
  { id:"lilac", name:"Lila",    bg:"#F0ECF9", emoji:"🟣" },
  { id:"dark",  name:"Oscuro",  bg:"#16202E", emoji:"⚫" },
];
function inkFor(bg) {
  const c = bg.replace("#","");
  const r=parseInt(c.slice(0,2),16), g=parseInt(c.slice(2,4),16), b=parseInt(c.slice(4,6),16);
  const lum = (0.299*r+0.587*g+0.114*b)/255;
  return lum < 0.5 ? "#F2F2F2" : "#0D1B2A";
}
function cardFor(bg) {
  const c = bg.replace("#","");
  const r=parseInt(c.slice(0,2),16), g=parseInt(c.slice(2,4),16), b=parseInt(c.slice(4,6),16);
  const lum = (0.299*r+0.587*g+0.114*b)/255;
  return lum < 0.5 ? "#202D3D" : "#FFFFFF";
}
function lineFor(bg) {
  const c = bg.replace("#","");
  const r=parseInt(c.slice(0,2),16), g=parseInt(c.slice(2,4),16), b=parseInt(c.slice(4,6),16);
  const lum = (0.299*r+0.587*g+0.114*b)/255;
  return lum < 0.5 ? "#2E3D52" : "#EDE8DC";
}
...
export default function App() {
  ...
  const [bgColor, setBgColor]   = useState(() => load("bgColor",  THEMES[0].bg));
  ...
  const ink  = inkFor(bgColor);
  const card = cardFor(bgColor);
  const line = lineFor(bgColor);
  const subtleText = ink === "#F2F2F2" ? "#9FB0C3" : T.slate;

  // ══════════════════════════════════════════════════════════
  return (
    <div style={{ fontFamily:"'Inter','Segoe UI',sans-serif",background:bgColor,minHeight:"100svh",color:ink,transition:"background .25s" }}>
      ...
      <header style={{ background:T.navy,padding:"16px 20px 0",position:"sticky",top:0,zIndex:10 }}>
        ...
        <div style={{ display:"flex",gap:2 }}>
          {[["semana","📆"],["materias","📚"],["objetivos","🎯"],["rachas","🔥"],["ajustes","⚙️"]].map(([k,ic])=>(
            <button key={k} onClick={()=>setTab(k)} style={{
              flex:1,padding:"9px 2px",borderRadius:"8px 8px 0 0",border:"none",cursor:"pointer",
              fontWeight:600,fontSize:10.5,
              background:tab===k?bgColor:"transparent",
              color:tab===k?(ink==="#F2F2F2"?"#fff":T.navy):"#8BA0BE",transition:"all .15s",
      ...
      <main style={{ padding:"18px 16px 80px",maxWidth:520,margin:"0 auto" }}>
        ...
        {tab==="semana" && <>
          ...
          <div style={{ display:"flex",gap:8,marginBottom:18,overflowX:"auto",paddingBottom:2 }}>
            {DAYS_SHORT.map((d,i)=>(
              <button key={i} onClick={()=>setDay(i)} style={{
                ...
                borderColor:activeDay===i?T.amber:line,
                background:activeDay===i?T.navy:card,
                color:activeDay===i?card:ink,
          ...
          {/* Timeline */}
          <div style={{ background:card,borderRadius:18,padding:"16px 12px 16px 48px",boxShadow:"0 2px 16px rgba(27,43,75,.07)",position:"relative",minHeight:mpx(DAY_END-DAY_START)+32 }}>
            {/* Hour lines */}
            {Array.from({length:(DAY_END-DAY_START)/60+1},(_,i)=>(
              <div key={i} style={{ position:"absolute",left:0,top:mpx(i*60)+16,right:0 }}>
                <span style={{ position:"absolute",left:4,fontSize:10,color:subtleText,transform:"translateY(-50%)",fontVariantNumeric:"tabular-nums" }}>{String(DAY_START/60+i).padStart(2,"0")}:00</span>
                <div style={{ position:"absolute",left:44,right:10,height:1,background:line,top:0 }} />
              </div>
            ))}

            {dayEvents.length===0 && (
              <div style={{ position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",color:subtleText }}>
              ...
              return (
                <div key={ev.id} onClick={()=>{setEditing({...ev,_day:activeDay});setModal("event");}} style={{
                  ...
                  <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                    ...
                    <span style={{ fontWeight:700,fontSize:13,color:ink }}>{label}</span>
                  </div>
                  {height>38 && <div style={{ fontSize:11,color:subtleText,marginTop:2 }}>{ev.time} · {ev.duration} min{ev.room?` · ${ev.room}`:""}</div>}
                </div>
              );
            })}
          </div>
        ...
        {tab==="materias" && <>
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            {subjects.map(s=>(
              <div key={s.id} style={{ background:card,borderRadius:18,overflow:"hidden",boxShadow:"0 2px 14px rgba(27,43,75,.06)" }}>
                <button onClick={()=>setExpanded(expanded===s.id?null:s.id)} style={{ width:"100%",background:"none",border:"none",padding:"16px 18px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",textAlign:"left" }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700,fontSize:15,color:ink }}>{s.name}</div>
                    <div style={{ fontSize:12,color:subtleText }}>{s.teacher}</div>
                    <div style={{ marginTop:8,height:5,borderRadius:3,background:line,overflow:"hidden" }}>
                      <div style={{ height:"100%",width:`${s.progress}%`,background:s.color,borderRadius:3,transition:"width .4s" }} />
                    </div>
                    <div style={{ fontSize:11,color:subtleText,marginTop:3 }}>Temario: {s.progress}% completado</div>
                  </div>
                  <span style={{ fontSize:18,color:subtleText,display:"inline-block",transform:expanded===s.id?"rotate(180deg)":"none",transition:"transform .2s" }}>▾</span>
                </button>
                {expanded===s.id && (
                  <div style={{ borderTop:`1px solid ${line}`,padding:"14px 18px 18px" }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
                      <div style={{ fontWeight:700,fontSize:11,color:subtleText,letterSpacing:".08em",textTransform:"uppercase" }}>Temario</div>
                    ...
                    {(s.topics||[]).map((topic,idx)=>{
                      ...
                      return (
                        <div key={idx} style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:idx<s.topics.length-1?`1px solid ${line}`:"none" }}>
                          <div style={{ width:22,height:22,borderRadius:6,background:done?s.color:line,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0,color:card }}>{done?"✓":""}</div>
                          <span style={{ fontSize:14,color:done?ink:subtleText,fontWeight:done?500:400 }}>{topic}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        {tab==="objetivos" && <>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            {goals.map(g=>{
              ...
              return (
                <div key={g.id} style={{ background:g.done?T.mintSoft:card,border:`2px solid ${g.done?T.mint:line}`,borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:14,transition:"all .15s" }}>
                  <button onClick={()=>toggleGoal(g.id)} style={{ width:26,height:26,borderRadius:8,flexShrink:0,border:`2.5px solid ${g.done?T.mint:line}`,background:g.done?T.mint:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:card,cursor:"pointer" }}>{g.done?"✓":""}</button>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600,fontSize:14,color:ink,textDecoration:g.done?"line-through":"none",opacity:g.done?.6:1 }}>{g.text}</div>
                    <div style={{ display:"flex",gap:8,marginTop:5,flexWrap:"wrap" }}>
                      {subj?<span style={{ fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:10,background:subj.color+"22",color:ink }}>{subj.icon} {subj.name}</span>
                           :<span style={{ fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:10,background:T.coralSoft,color:ink }}>🎯 Personal</span>}
                      <span style={{ fontSize:11,color:subtleText }}>📅 {g.due}</span>
                    </div>
                  </div>
                  <button onClick={()=>{setEditing(g);setModal("goal");}} style={{ background:"none",border:"none",cursor:"pointer",fontSize:16,color:subtleText }}>✏️</button>
                </div>
              );
            })}
          </div>
        ...
        {tab==="rachas" && <>
          <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
            {habits.map(h=>{
              ...
              return (
                <div key={h.id} style={{ background:card,borderRadius:18,padding:"18px 18px 14px",boxShadow:"0 2px 14px rgba(27,43,75,.06)" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:14 }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:700,fontSize:15,color:ink }}>{h.name}</div>
                      <div style={{ fontSize:12,color:subtleText }}>Meta: {h.goal} días/semana</div>
                    </div>
                    <div style={{ textAlign:"center" }}>
                      <div style={{ fontWeight:800,fontSize:28,color:s>0?T.amber:subtleText,lineHeight:1 }}>{s}</div>
                      <div style={{ fontSize:10,color:subtleText }}>🔥 racha</div>
                    </div>
                    <button onClick={()=>{setEditing(h);setModal("habit");}} style={{ background:"none",border:"none",cursor:"pointer",fontSize:16,color:subtleText }}>✏️</button>
                  </div>
                  ...
                  <div style={{ display:"flex",gap:6,marginBottom:14 }}>
                    {["L","M","M","J","V","S","D"].map((d,i)=>(
                      <div key={i} style={{ flex:1,textAlign:"center" }}>
                        <div style={{ fontSize:10,color:subtleText,marginBottom:4 }}>{d}</div>
                        <div style={{ width:"100%",aspectRatio:"1",borderRadius:8,background:wk[i]?h.color:line,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:card }}>
                          {wk[i]?"✓":""}
                        </div>
                      </div>
                    ))}
                  ...
                  <button onClick={()=>toggleHabit(h.id)} style={{
                    width:"100%",padding:"11px",borderRadius:12,border:`2px solid ${checkedToday?h.color:line}`,
                    background:checkedToday?h.soft:"transparent",
                    fontWeight:700,fontSize:14,color:checkedToday?ink:subtleText,cursor:"pointer",transition:"all .2s",
                  }}>
        ...
        {/* ══ AJUSTES ══ */}
        {tab==="ajustes" && <>
          <div style={{ background:card,borderRadius:18,padding:"18px 18px 20px",boxShadow:"0 2px 14px rgba(27,43,75,.06)",marginBottom:14 }}>
            <div style={{ fontWeight:700,fontSize:15,color:ink,marginBottom:4 }}>🎨 Color de fondo</div>
            <div style={{ fontSize:12,color:subtleText,marginBottom:16 }}>Elegí un tema o tu propio color</div>

            {/* Preset themes */}
            <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:18 }}>
              {THEMES.map(th=>(
                <button key={th.id} onClick={()=>setBgColor(th.bg)} style={{
                  display:"flex",flexDirection:"column",alignItems:"center",gap:6,
                  padding:"12px 6px",borderRadius:14,cursor:"pointer",
                  border:`2.5px solid ${bgColor===th.bg?T.amber:line}`,
                  background:th.bg,
                }}>
                  <span style={{ fontSize:20 }}>{th.emoji}</span>
                  <span style={{ fontSize:11,fontWeight:700,color:inkFor(th.bg) }}>{th.name}</span>
                  {bgColor===th.bg && <span style={{ fontSize:10,color:T.amber,fontWeight:800 }}>✓ Activo</span>}
                </button>
              ))}
            </div>

            {/* Custom color picker */}
            <div style={{ fontWeight:700,fontSize:12,color:subtleText,marginBottom:8,textTransform:"uppercase",letterSpacing:".06em" }}>Color personalizado</div>
            <div style={{ display:"flex",alignItems:"center",gap:12 }}>
              <input
                type="color"
                value={bgColor}
                onChange={e=>setBgColor(e.target.value)}
                style={{ width:54,height:54,borderRadius:14,border:`2px solid ${line}`,cursor:"pointer",padding:0,background:"none" }}
              />
              <div style={{ flex:1 }}>
                <input
                  type="text"
                  ...
                  style={{ ...inp, background:bgColor.length===7?"transparent":undefined, color:ink, borderColor:line }}
                  placeholder="#F7F4EE"
                />
                <div style={{ fontSize:11,color:subtleText,marginTop:6 }}>Tocá el círculo para abrir la paleta completa</div>
              </div>
            </div>

            <button onClick={()=>setBgColor(THEMES[0].bg)} style={{ ...btn("transparent",T.coral),border:`1.5px solid ${T.coralSoft}`,marginTop:18 }}>
